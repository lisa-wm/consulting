...
\textbf{Word.} Words $w$ are sequences of characters.
While they typically represent the smallest unit of text considered, BERT may 
decompose documents into even smaller parts. 
% \\
...
projects the data into a subspace spanned by $F$'s principal eigenvectors, and
...
modeling.
In this, it also provides the foundation for the STM.
...
\textit{polarities}.
As such, sentiment analysis is a standard classification problem
...
The actual class label predictions $\hat y \in \mathcal{Y}$ are then found by 
...
(e.g., \citet{zuoetal2016}, \citet{quiangetal2019}).
