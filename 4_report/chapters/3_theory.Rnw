...
In particular, $K$ is a hyperparameter that must be specified \textit{a priori}
\citep{aggarwal2018}.
\\
...
(\citet{nguyenetal2018}, \citet{tianetal2021}, \citet{wangetal2021}).
While theoretical arguments are strong, we decide against simultaneous 
methods mostly due to the complexity of such approaches, which is at odds with 
...
For the standard machine learning classification we use topic modeling, 
...
In the deep learning solution we experiment with sentiments that directly refer 
...
We will discuss this finding in section \ref{discussion} and now proceed with 
