...
\in \R^{D \times p}$ and let $\bm{y} \in \{0, 1\}^D$ denote 
...
\in \R^{h + 1}$, $h \in \N$\footnote{
It might be that $h = p$, but this is not necessarily the case (e.g., in 
...
regularizers are balanced by a convex combination with parameter $\alpha \in 
[0, 1]$.
We then arrive at the following optimization problem
\citep{hastieetal2021}:

\begin{equation*}
    \min_{(\gamma_0, \bm{\gamma}) \in \R^{h + 1}} - \frac{1}{D} \sum_{d = 1}^D
...
outer (performance evaluation) and inner (hyperparameter tuning) optimization 
loop.
...
More efficient strategies might improve the tuning process here but their use is 
not straightforward.
...
not as simple due to the heterogeneity of the 500 base learners and
...
in a specific way.
To this end, we split words into tokens or word fragments based on a given 
...
eventually thresholded to yield a class label \citep{xuetal2019}.
\\
...
Furthermore, we enhance our basic model by post-training it on domain knowledge, 
