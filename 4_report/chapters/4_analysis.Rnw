...
$f(\bm{x}) = \sum_{t = 1}^T r_t \mathbb{I}[\bm{x} \in R_t]$, where $T$ denotes 
the number of terminal regions $R_1, R_2, \dots, R_T$ and 
...
usually at a cut-off value of 50\%.
By application of the logit transformation we obtain the linear 
predictor $\gamma_0 + \bm{\gamma}^T\bm{x}_d = \log 
\tfrac{\pi(\bm{x}_d)}{1 - \pi(\bm{x}_d)}$.

Here, $(\gamma_0, \bm{\gamma})$ is estimated by minimizing a penalized version 
...
$\lambda \geq 0$ and $\alpha \in [0, 1]$ are hyperparameters that need to be 
tuned \citep{hastieetal2021}:

\begin{equation*}
    \min_{(\gamma_0, \bm{\gamma}) \in \R^{p + 1}} - \frac{1}{n} \sum_{d = 1}^D
...
We evaluate performance of two variants of our pipeline, a full version 
...
\begin{enumerate}
  \item Accuracy: $\rho_{\text{acc}} =  \frac{\text{TN + TP}}{
  \text{TN + TP + FN + FP}}$, and
  \item F1 score: $\rho_{\text{F1}} =  2 \cdot \dfrac{\frac{\text{TP}}{
  \text{TP + FP}} \cdot 
  \frac{\text{TP}}{\text{TP + FN}}}{\frac{\text{TP}}{\text{TP + FP}} +
...
We see from table \ref{tab_res_ml} that both algorithms achieve rather low 
cross-validated performance in the realm of 72\% accuracy.
Considering the fact that these values are remarkably close to the share of 
negative labels in the data, it becomes clear that neither is able to outperform 
a random classifier assigning classes according to the empirical relative class 
frequencies (without regard to any covariates, referred to as \textit{
featureless learner}).
The entire feature extraction procedure must thus be regarded obsolete.

\begin{table}[H]
  \small
  \begin{tabular}{l|r|r|r}
    \textbf{metric} & \textbf{learner with topic modeling} & 
    \textbf{learner without topic modeling} & \textbf{featureless learner} \\
    \hline
    accuracy & 0.7235 & 0.7292 & 0.7243 \\
    \hline
    F1 score & 0.8393 & 0.8399 & 0.8398 \\
    \hline
    TN & 293.0000 & 288.0000 & 293.0000 \\
    \hline
    TP & 0.0000 & 7.3333 & 0.0000 \\
    \hline
    FN & 111.6667 & 104.3333 & 111.6667 \\
    \hline
    FP & 0.3333 & 5.3333 & 0.0000 \\
  \end{tabular}
...
It is, moreover, worth noting that the learner without topic clustering performs 
slightly better (if only by a very small margin) than the one using 
topic-specific embeddings.
...
All in all, it appears that the handcrafted features by which we represent 
our texts are not helpful in sentiment classification for these data.
...
Transformer-based models, which have gained much traction in recent NLP 
...
belief we exploit in evaluating the generalization ability of our learner in a 
reliable way.
...
Still, the tasks might not be fundamentally different, and some of the gathered 
information on the feature-target relationship will be of use nonetheless.
Much like in human learning processes we can presume a certain amount of 
abstraction.
It is precisely this migration of previous knowledge that is at the heart of 
...
Disadvantages of recurrent modeling, however, include a lack of parallelization 
and the tendency to favor recent input.
The attention mechanism allows the network to access any past modeling state by
allocating dedicated weights and therefore achieves much better 
...
a large corpus in a specific language with the help of neural networks. In the 
...
representation the latter maps to predictions \citep{zhang2020}.
...
sentence.
In short, the document contents and structure are translated in to a numerical 
representation.
The inputs have to be of same length, where the maximum length of an input 
...
During training, half of the inputs is comprised of original pairs, while the 
other half has of a random sentence from the corpus as second sentence. 
...
in particular.
In order to have the pre-trained network solve a specific problem, we need to 
fine-tune it on the target task.
Crucially, at this point, the need for labeled data comes in.
For fine-tuning we simply need to alter the output layer that adapts to the 
target task.
...
classification.
During fine-tuning, all BERT parameters are updated, though \citep{devlin2019}.
...
Lastly, the aspect sentiment classification task attempts to classify the 
sentiment polarity of a given text, generally into positive, negative or 
neutral but omitting the neutral category in our case. 
...
we obtain probabilities predicted for each of the sentiment categories 
\citep{xuetal2019}.
\\
...
Finally, a fourth model is given by another German-language model 
...
source than the basic model: recent Wikipedia dump, EU Bookshop corpus, Open 
Subtitles, CommonCrawl, ParaCrawl and News Crawl. 
The dataset has a size of 16 GB and over 2 billion tokens, promising even better 
results.

For ABSA we additionally post-train the basic model on the GermEval 
...
and is applied on tweets with political context in this project, which is 
