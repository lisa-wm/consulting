...
nature.
We therefore build a pipeline for sentiment classification that allows to repeat 
training procedures and exchange components easily.
...
\begin{enumerate}
  \item Topic-specific embeddings are computed globally across several 
  observations.
  ...
  prediction.
  \item Automating (parts of) algorithm selection and hyperparameter tuning 
  helps with crucial design decisions by attaching them to the given data.
  \item The possibility to plug in alternative components makes the 
