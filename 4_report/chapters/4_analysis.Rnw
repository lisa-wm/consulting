...
\begin{minipage}[b]{0.45\textwidth}
  A number of components make up a typical workflow in ML 
  (figure \ref{fig_ml_workflow}).
  Task, learner, performance measure
  Makes sense to build this as a modular system (graph leaner) because 1. this 
  enables us to quickly exchange components and 2. tuning and evaluation require 
  us to repeat the learning process over and over
  therefore we design this as a pipeline, propose a certain design but everything 
...
\begin{minipage}[b]{0.5\textwidth}
  \begin{figure}[H]
    ...
    \end{center}
  \caption[ML workflow]{Typical ML workflow \citep{mlr3book}}
...
provides tabular data which then enter an automated ML (AutoML) pipeline (see 
section \ref{pipeline}). 
...
They mainly capture information related to sentiment analysis.
Dynamic features, by contrast, are directed at topic modeling and therefore 
computed globally across several observations.
Since unbiased learning requires strict separation between train and test 
sphere, we must be careful to wall off predictions at test time from any 
computation during training.
This is achieved by integrating the extraction of dynamic features into the 
AutoML pipeline such that they are every
\\
