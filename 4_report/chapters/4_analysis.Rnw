...
When allowed to full growth, i.e., until each observation has its own leaf node, 
...
receives a bootstrap sample of the data for training, and second, it is allowed
to use only a random subset of the features for splitting.
