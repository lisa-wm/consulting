...
As this requires strict separation between train and test sphere in computation, 
the distinction between static and dynamic variables is mirrored by the analytic 
process through integrating the extraction of dynamic features into the AutoML 
pipeline.
\\
