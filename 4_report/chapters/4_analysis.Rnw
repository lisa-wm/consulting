...
and thus on average achieves an accuracy in the realm of the share of negative
observations:
...
number of embedding vectors is created (factor 6 for the extreme case of $K$), 
spanning a quite high-dimensional feature space for relatively few observations.
...
not straightforward due to the heterogeneity of the 500 base learners and
the randomness induced by sampling features as well as tuning the size of this
sample via random search.
...
classifier (which are actually quite alike, as discussed above). 
