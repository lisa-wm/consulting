...
\textbf{Static vs dynamic features} \\
...
Dynamic features, by contrast, are computed globally across different 
observations, which requires strict adherence to the untouched-test-set 
principle.
...
\begin{enumerate}
  ...
  \citep{sentiws} and a collection by \citet{rauh2018} for word polarities and 
  an emoji polarity list accumulated by \citet{kraljetal2015}.
  In each tweet, if a word or emoji is part of the dictionary and judged to have 
  either positive or negative connotation, it raises the respective polarity 
  count by one.
  ...
  For hashtags we resort to the same naive measure since we found them to be so 
  heterogeneous across documents that no meaningful information could be 
  extracted. 
  This certainly marks an opportunity for future improvement.
  
  \item \textbf{Twitter variables.}
  
\end{enumerate}

The above feature extraction steps rely heavily on \texttt{R}'s 
...
We also use lists of stopwords in multiple places to exclude uninformative 
tokens such as articles or auxiliary verbs.
These are again compilations from various sources, including \texttt{quanteda}'s 
built-in list, open-source data from \citet{stopwords_iso} and some manually 
