...
Website contents do not come in nice tabular form -- we need to access them using HTML nodes / CSS selectors.
...
Once we have found this (which might take a while if the website structure if nested, so prepare for some trial-and-error here), `rvest` makes the rest rather convenient:
...
Extract years without parentheses (more on regular expressions coming soon) and convert to numeric: 
...
### Store scraped data in a data table

Now we have everything we need and can store the data in an R-friendly object.

We will use a `data.table` here. Some people may be more familiar with `data.frame` objects but `data.table` is rather more powerful and substantially faster for large data sets, which is why we use it a lot in our code (note that the syntax is a bit different).

```{r create_df}
imdb_scraped_data <- data.table::data.table(
