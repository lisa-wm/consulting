...
* `unemployment_rate` as a smooth effect with 5 degrees of freedom, and
* `share_pop_migration` as a smooth effect with 5 degrees of freedom.

The resulting formula, `~ party + bundesland + s(unemployment_rate, df = 5) + s(share_pop_migration, df = 5)`, is perfectly disputable -- this decision is up to domain knowledge and some trial-and-error.
...
Seems `r toString(n_topics)` topics is a good choice, so let's fit the actual **model** now:
