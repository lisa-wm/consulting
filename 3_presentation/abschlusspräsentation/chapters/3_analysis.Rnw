...
\begin{frame}<0>[noframenumbering]{Structural topic model (STM)}
...
% \vfill
% 
% \textit{Die Ausgrenzung von MigrantInnen von der \#EssenerTafel 
% ist inakzeptabel und \\rassistisch.
% Wir dürfen nicht zulassen, dass die Ärmsten gegeneinander 
% \\ausgespielt werden.}
...
\textbf{Labels:} \textit{MigrantInnen, rassistisch, Ärmsten}
\end{minipage}}
...
ausgespielt werden. [SEP]
\\
\textbf{Label:} \textit{IsNextSentence}
\end{minipage}}
...
\\
\textbf{Label:} \textit{IsNotNextSentence}
\end{minipage}}
