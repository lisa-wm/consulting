...
filepath <- paste0("./output/", file, ".rds")
...
if (grepl("monthly", file)) {
  ...
  data <- data %>% 
    select(-c("Jahr", "Monat"))
  docid_field <- "docid"
  corp_topic <- quanteda::corpus(x = data, text_field = "Tweets_Dokument", docid_field = docid_field)
} else {
...
filepath_out <- paste0("./output/", file_new, ".rds")
